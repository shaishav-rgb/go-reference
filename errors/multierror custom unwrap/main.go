...
func main() {
	...
	case interface{ Unwrap() []error }:
		...
		fmt.Println(innerErrs) // print does not call individual Error() method on the error slice, this just prints the []error slice
		for _, innerErr := range innerErrs {
			// process each innerErr
			fmt.Println(innerErr)  // this calls Error() method in individual error.
		}
	default:
